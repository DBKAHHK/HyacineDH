...
public class HandlerSendMsgCsReq : Handler
{
    public override async Task OnHandle(Connection connection, byte[] header, byte[] data)
    {
        ...
        await connection.Player!.FriendManager!.SendInviteMessage(connection.Player!.Uid, (int)req.TargetList[0],
                new LobbyInviteInfo { CELMKOLBJNN = new LKAPFHAHNEM { PanelId = (uint)req.DIJGJPGFFLG.PAJIMKAGAPI.COPGBEFIKGE, ModifierContentType = req.DIJGJPGFFLG.PAJIMKAGAPI.GCLCCOGIDOA, CFDANMOMHPI = req.DIJGJPGFFLG.PAJIMKAGAPI.GBAHHHPMLDK } });
